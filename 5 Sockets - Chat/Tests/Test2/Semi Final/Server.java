...
public class Server extends JFrame implements Runnable{  
	...
	public void run(){  
		while (thread != null){  
			...
			catch(IOException ioe){ txtStatus.append("### Server accept error: " + ioe + "\n");}
		}
	}
	public void start(){
		if (thread == null){  
	...
	public synchronized void remove(int ID){  
		...
		if (pos >= 0){  
			...
			catch(IOException ioe){ txtStatus.append("### Error closing thread: " + ioe + "\n"); }
			toTerminate.stop(); 
		}
	}
	private void addThread(Socket SOCK){  
		if (clientCount < clients.length){  
			...
			catch(IOException ioe){ txtStatus.append("### Error opening thread: " + ioe + "\n");} 
		}
		else{txtStatus.append("*** Client "+ClientIP+" refused! Maximum " + clients.length + " reached.\n");}
	}
